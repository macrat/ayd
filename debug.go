...
func debugStatusReporter() {
	...
	fmt.Fprintf(f, "timestamp\tcurrent_goroutines\tcurrent_heap_bytes\tnum_mallocs\tnum_frees\n")
	...
	for range t {
		...
		fmt.Fprintf(f, "%s\t%d\t%d\t%d\t%d\n", time.Now().Format(time.RFC3339), runtime.NumGoroutine(), mem.HeapAlloc, mem.Mallocs, mem.Frees)
	}
}
